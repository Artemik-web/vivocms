...
<div class="panel">
  ...
  <div class="heading">
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item><a href="#/app/productcate">分类管理</a></el-breadcrumb-item>
  ...
  <div class="search">
    <div class="options">
      ...
      <el-input v-model.trim="keyword" placeholder="请输入分类名称" size="small" clearable />
    </div>
  ...
  >
    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="100px">
      ...
      <el-form-item label="上传图片" prop="picurl">
        <el-tooltip class="item" effect="dark" content="请上传530x180的图片尺寸！" placement="right-start">
          <el-upload
          class="pic-upload size200x68"
...
export default {
  ...
  computed: {
    searchData () {
      return this.tableData.filter(item => {
        ...
        const cd2 = this.keyword !== '' ? searchCd(this.keyword, item.title) : true // 条件2
        return cd1 && cd2
      })
    }
  },
  methods: {
    ...
    handelChange (state, id) {
      this.$http.post('productcate/state', { state, id }).then(res => {
        if (res.result.err > 0) {
    ...
    delItem (id) {
      this.$confirm('您确定要删除此条记录吗？', '提示').then(() => {
        this.$http.post('productcate/remove', { id }).then(res => {
          if (res.result.err > 0) {
            this.$message.error(res.result.msg)
          } else {
            this.$message.success(res.result.msg)
    ...
    batchDel () {
      this.$confirm('您确定要删除所选的记录吗？', '提示').then(() => {
        // 发送删除请求
        this.$http.post('productcate/remove', { id: this.ids }).then(res => {
          if (res.result.err > 0) {
            this.$message.error(res.result.msg)
          } else {
            this.$message.success(res.result.msg)
    ...
    save () {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.$http.post('productcate/save', { ...this.form }).then(res => {
            if (res.result.err > 0) {
              this.$message.error(res.result.msg)
            } else {
              this.$message.success(res.result.msg)
